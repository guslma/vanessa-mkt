  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <form onSubmit={handleSubmit} className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-xl bg-white p-5 shadow-xl">
        <h2 className="text-lg font-semibold text-slate-800">{empreendimento ? 'Editar empreendimento' : 'Novo empreendimento'}</h2>
        ...
        <div className="mt-3 grid grid-cols-2 gap-3">
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700">Fase Atual</label>
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700">Data de Lançamento</label>
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700">Responsável Comercial</label>
            <input value={form.responsavel_comercial ?? ''} onChange={(e) => setForm({ ...form, responsavel_comercial: e.target.value })} className={inputClass()} />
          </div>
        </div>

        <label className="mt-3 block text-sm font-medium text-slate-700">Endereço</label>
        <input
          value={form.endereco ?? ''}
        ...
        <label className="mt-3 block text-sm font-medium text-slate-700">Link / Pasta de Materiais</label>
        ...
        {empreendimento && (
          <div className="mt-4 rounded-lg bg-slate-50 p-3">
            <p className="text-xs font-medium text-slate-600">Link público (somente leitura, sem login)</p>
            <div className="mt-1.5 flex items-center gap-2">
              ...
              {user?.role === 'admin' && (
                <button
                  ...
                  className="flex items-center gap-1.5 rounded-lg border border-slate-300 px-2.5 py-1 text-xs text-slate-600 hover:bg-white disabled:opacity-50"
                >
