...
export function TarefaComentarios({ tarefaId }: { tarefaId: string }) {
  ...
  return (
    <div className="mt-5 border-t border-slate-100 pt-4">
      ...
      {!isLoading && comentarios.length === 0 && <p className="mt-2 text-xs text-slate-400">Nenhum comentário ainda.</p>}
          ...
          return (
            <li key={c.id} className="rounded-lg bg-slate-50 px-3 py-2 text-sm">
              <div className="flex items-start justify-between gap-2">
                ...
                {canDelete && (
                  <button type="button" onClick={() => remove.mutate(c.id)} className="shrink-0 text-slate-400 hover:text-red-600">
                    <Trash2 size={13} />
                  </button>
                )}
              </div>
              <p className="mt-1 text-xs text-slate-400">{c.user_name ?? 'Usuário removido'} · {formatDateTime(c.created_at)}</p>
            </li>
          );
        })}
      </ul>
