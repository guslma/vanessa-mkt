...
export function TarefaHistory({ tarefaId }: { tarefaId: string }) {
  ...
  return (
    <div className="mt-5 border-t border-slate-100 pt-4">
      ...
      {!isLoading && eventos.length === 0 && <p className="mt-2 text-xs text-slate-400">Sem atividade registrada ainda.</p>}
      <ul className="mt-2 flex max-h-32 flex-col gap-1.5 overflow-y-auto text-xs">
        {eventos.map((ev) => (
          <li key={ev.id} className="text-slate-500">
            <span className="text-slate-400">{formatDateTime(ev.created_at)}</span>
            {' · '}
            <span className="font-medium text-slate-600">{ev.user_name ?? 'Sistema'}</span>
            {' — '}{ev.descricao}
          </li>
        ))}
      </ul>
