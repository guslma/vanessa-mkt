  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <form onSubmit={handleSubmit} className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-xl bg-white dark:bg-slate-800 p-5 shadow-xl">
        <h2 className="text-lg font-semibold text-slate-800 dark:text-slate-100">{tarefa ? 'Editar tarefa' : prefill ? 'Duplicar tarefa' : 'Nova tarefa'}</h2>

        <label className="mt-3 block text-sm font-medium text-slate-700 dark:text-slate-200">Empreendimento</label>
        ...
        <label className="mt-3 block text-sm font-medium text-slate-700 dark:text-slate-200">Tarefa / Atividade</label>
        ...
        <div className="mt-3 grid grid-cols-2 gap-3">
          <div>
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Categoria</label>
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Responsável</label>
            <input value={form.responsavel ?? ''} onChange={(e) => setForm({ ...form, responsavel: e.target.value })} className={inputClass()} />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Prioridade</label>
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Data Início</label>
            <input type="date" value={form.data_inicio ?? ''} onChange={(e) => setForm({ ...form, data_inicio: e.target.value || null })} className={inputClass()} />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Prazo</label>
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Data Conclusão</label>
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Repetir</label>
        ...
        {form.recorrencia !== 'nenhuma' && (
          <p className="mt-1.5 text-xs text-slate-500 dark:text-slate-400">Ao marcar essa tarefa como concluída, uma nova cópia é criada automaticamente com o prazo avançado.</p>
        )}
